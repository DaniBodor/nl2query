# `nl2query` developer documentation
...
# install nl2query as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `nl2query`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d nl2query.XXXXXX)
git clone git@github.com:DaniBodor/nl2query .
...
Visit
[https://test.pypi.org/project/nl2query](https://test.pypi.org/project/nl2query)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d nl2query-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple nl2query
```
...
Don't forget to also make a [release on GitHub](https://github.com/DaniBodor/nl2query/releases/new).